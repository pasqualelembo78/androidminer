...
class XMRigForAndroid(context: ReactApplicationContext) : ReactContextBaseJavaModule(context) {
    ...
    fun start(configurationJSON: String) {
        ...
        // reset() non necessario: setConfiguration() sovrascrive interamente il config
        // con il JSON ricevuto da JS (già completo). Lasciarlo causava la perdita dei
        // profili thread espliciti in caso di fallback sul template asset.
